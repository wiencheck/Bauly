...
**Bauly** is written in Swift 5 and supports iOS 10.0+
...
### Usage
**Bauly** is managed via a singleton
```
Bauly.shared
```
It manages internally a queue of pending banners. Only one banner can be displayed at the time.
...
```
func forcePresent(title: String, 
                subtitle: String?, 
                icon: UIImage?, 
                duration: TimeInterval, dismissAfter delay: TimeInterval, 
                in window: UIWindow?, 
                feedbackStyle: UIImpactFeedbackGenerator.FeedbackStyle?, pressHandler: (() -> Void)?, 
                completionHandler: (() -> Void)?)
```
